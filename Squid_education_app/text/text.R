fullmodelTxt <- list(
  ...
  "modelEquation_1"     = "The equation represents the model that generated phenotypic values `r  paste0('$',EQ$phen.1,'$')` or `r  paste0('$',EQ$phen.2,'$')`) of 
                           two traits for each individual `r  paste0('$',NOT$ind,'$')` and at each instance of time `r  paste0('$',NOT$time,'$')`, in each group `r  paste0('$',NOT$group,'$')`.",
  "modelEquation_2"     = "It should be noted that subscripts are organised to represent the different hierarchical 
                           levels involved in the generation of the value of the trait: subscript `r  paste0('$',NOT$time,'$')` stands for the 
                           instance at which the traits is measured for the individual `r  paste0('$',NOT$ind,'$')`, within group `r  paste0('$',NOT$group,'$')`. 
                           Finally subscript `r  paste0('$',NOT$trait.1,'$')` or `r  paste0('$',NOT$trait.2,'$')` refers to the two traits generated.",
  "modelEquation_3"     = "At first sight we can see that the phenotypic value of the trait `r  paste0('$',NOT$trait.1,'$')` for individual `r  paste0('$',NOT$ind,'$')` 
                          at instance `r  paste0('$',NOT$time,'$')` in group `r  paste0('$',NOT$group,'$')` is the sum of a series of components. 
  ...
  "modelEquation_4"     = "The first component on the right hand side of the equation represents the population average 
                          `r  paste0('$',EQ$mean0.1,'$')` for the trait `r  paste0('$',NOT$trait.1,'$')`, and the deviation of each individual's trait mean 
                          from that population average `r  paste0('$',EQ$dev0.1,'$')`, generally called individual level or individual intercept). 
                          All the other elements occurring on the right hand side of the equation represent also deviations 
                          from the average of the trait in that population.",
  "modelEquation_5"     = "Let's now go to the end of the equation, where we find the residual `r  paste0('$',EQ$error.1,'$')`. 
													...
													At the left of the 
                          residual term we have `r  paste0('$',EQ$group.1,'$')` that represents the deviation from the population average of 
                          trait `r  paste0('$',NOT$trait.1,'$')` for a group `r  paste0('$',NOT$group,'$')` (this means that all the individuals of a given group `r  paste0('$',NOT$group,'$')` share 
                          the same value of `r  paste0('$',NOT$groupV,'$')` `r  paste0('$',NOT$groupV,'$')` follows a normal distribution with a mean = 0 and variance = `r  paste0('$Var(',NOT$groupV,')$')`).
                          The concept of grouping is wide ranging from genetic similarities (e.g. families) to spatial aggregations 
                          (e.g. territories) and diet compositions and could be specific to the each study.",
  "modelEquation_6"     = "The last three components `r  paste0('$(',EQ$mean1.1,'+',EQ$dev1.1,').',EQ$env1,'+(',EQ$mean2.1,'+',EQ$dev2.1,').',EQ$env2,' 
  ...
  "modelEquation_7"     = "These terms represent phenotypically plastic changes in the value of the trait as a function 
                          of two environmental factors `r  paste0('$',NOT$env,'_1$')` and `r  paste0('$',NOT$env,'_2$')`. In other words, that part of the equation is 
                          the reaction norm of trait `r  paste0('$',NOT$trait.1,'$')` as a function of either environment `r  paste0('$',NOT$env,'_1$')` or `r  paste0('$',NOT$env,'_2$')` 
                          (or both `r  paste0('$',NOT$env,'_1$')` and `r paste0('$',NOT$env,'_2$')`. Below we will explain the equation for a given environmental 
                          factor `r  paste0('$',NOT$env,'$')`. Environment `r  paste0('$',NOT$env,'$')` can be measured and thus you can decide whether or not to include `r  paste0('$',NOT$env,'$')` 
                          in subsequent mixed-effects models (whether you include `r  paste0('$',NOT$env,'$')` in the model or not allows you to 
                          test how unknown/unmeasured environments affect estimates of other parameters in the model). 
                          The reaction norm is itself composed of a population-level component (e.g. `r  paste0('$',EQ$mean1.1,'$')`) and of 
                          an individual deviation from the population norm `r  paste0('$',EQ$dev1.1,'$')`). 
                          When both terms are combined trait values are generated so that individuals differ 
                          in their slopes for trait `r  paste0('$',NOT$trait.1,'$')` as a function of environment `r  paste0('$',NOT$env,'$')`. 
                          Lastly there is a possibility to implement an interaction between `r  paste0('$',NOT$env,'_1$')` and `r  paste0('$',NOT$env,'_2', '[$(',EQ$mean12.1,'+',EQ$dev12.1,')',EQ$env12,'$].')`.",
  
  "indSpecResponses"    = "Below we show the way we generate the different values of each individual-level random effect for 
                          both traits `r  paste0('$',NOT$trait.1,'$')` and `r  paste0('$',NOT$trait.2,'$')`. The vector of individual deviation of the intercept and slopes for 
                          the two traits follows a multivariate normal distribution (MNV) with a zero-mean and covariance/variance 
                          matrix structure `r  paste0('$\\Omega_{',NOT$devI,NOT$devS,' }$')`. On the right hand side is shown (co)variance matrix 
                          `r  paste0('$\\Omega_{',NOT$devI,NOT$devS,' }$')`. Values of variance stand along the diagonal of the matrix (i.e., respectively, 
                          the among-individual variance in intercepts for `r  paste0('$',NOT$trait.1,'$')` `r  paste0('$(Var(',NOT$devI,'_',NOT$trait.1,'))$')`, the among-individual variance in slopes 
                          for `r  paste0('$',NOT$trait.1,'$')` `r  paste0('$(Var(',NOT$devS,'_',NOT$trait.1,'))$')`, the among-individual variance in intercepts for `r  paste0('$',NOT$trait.2,'$')` `r  paste0('$(Var(',NOT$devI,'_',NOT$trait.2,'))$')` 
                          and the among-individual variance in slopes for `r  paste0('$',NOT$trait.2,'$')` `r  paste0('$(Var(',NOT$devS,'_',NOT$trait.2,'))$')`. 
                          Covariance values between intercepts (e.g. `r  paste0('$Cov(', EQ2$dev0.1, ',', EQ2$dev0.2,')$')`), 
                          slopes (e.g. `r  paste0('$Cov(', EQ2$dev1.1, ',', EQ2$dev1.2,')$')`), 
                          and intercepts and slopes (e.g. `r  paste0('$Cov(', EQ2$dev0.1, ',', EQ2$dev1.1,')$')`) lie below the diagonal. 
                          ...
                          This is more convenient for users while correlations range between -1 and 1.",
  ...
  "environment_2"       = "For the first structure (<b>stochastic</b>), environmental values `r  paste0('$',NOT$env,'_{',NOT$random,NOT$time,'}$')` are 
                          generated using a Normal distribution with mean 0 and variance `r paste0('$Var(',NOT$env,'_',NOT$random,')$')`. 
                          $`r  paste0('$',NOT$env,'_{',NOT$random,NOT$time,'}\\sim N(0,Var(',NOT$env,'_',NOT$random,'))$')`
                          ...
                          The other options are more complex but also more realistic.",
  "environment_3"       = "For the <b>temporally auto-correlated</b> `r  paste0('$',NOT$env,'_{',NOT$autocorrelated,NOT$time,'}$')`
                          ...
                          Environmental values `r  paste0('$',NOT$env,'_{',NOT$autocorrelated,NOT$time,'}$')` are generated 
                          as the product of a stochastic value by a decay function.
                          `r  paste0('$',NOT$env,'_{',NOT$autocorrelated,NOT$time,'}\\sim N(0,Var(',NOT$env,'_',NOT$random,'))\\times e^{-\\alpha\\Delta ',NOT$time,'}$)`                          
                          where `r paste0('$\\Delta ',NOT$time,'$')` is the time interval between two instances of the study period, and $\\alpha$
                          is the decay rate, a measure of how fast the correlation decays with time.
                          $$\\alpha=ln(autocor)$$
                          where $autocor$ is the desired correlation between two successive values.",
  "environment_4"       = "Once the <b>linear</b> trend is set up SQuID generates 
                          a series of values following a classical linear equation.
                          `r  paste0('$',NOT$env,'_{',NOT$linear,NOT$time,'}=a+b.',NOT$time,'$')`
                          where $a$ and $b$ are respectively, the intercept and the slope of the regression between 
                          `r  paste0('$',NOT$env,'_{'',NOT$linear,NOT$time,'}$')` and `r  paste0('$',NOT$time,'$')`.  
                          This option allows you to generate environmental values that can change linearly with time, 
                          for example mimicking phenology during a season or long-term environmental changes.",
  "environment_5"       = "If you are interested in mimicking daily or seasonal fluctuations you can choose a <b>cyclic</b> 
                          (sinusoidal) environmental variation, according to a sinusoidal equation.
                          `r  paste0('$',NOT$env,'_{',NOT$cyclic,NOT$time,'}=a\\times sin(b.',NOT$time,'+c)+v$')`
                          ...
                          You can therefore change the shape of the cyclic curve by setting the different parameters.",
  ...
  "environment_7"       = "The environment could be also <b>shared</b> or <b>unshared</b> among individuals. 
                          ...
                          if the environment would vary over time. In this case, only one environment `r  paste0('$',NOT$env,'$')`) 
                          ...
                          in our population 3 different environments `r  paste0('$',NOT$env,'_{',NOT$ind,'=a}$')`, `r  paste0('$',NOT$env,'_{',NOT$ind,'=b}$')`, and `r  paste0('$',NOT$env,'_{',NOT$ind,'=c}$')` will be generated for 
  ...
  "environment_8"       = "Finally, in SQuID the final output of each environmental effect `r  paste0('$',EQ2$env1,'$')`, `r  paste0('$',EQ2$env2,'$')` and `r  paste0('$',EQ2$env12,'$')`) is 
													expressed in unit variance (i.e., `r  paste0('$Var(',NOT$env,')=1$')` and mean-centered (i.e., `r  paste0('$E(',NOT$env,')=0$)`.')",
  "groupingNerror"      = "The high-level grouping term `r  paste0('$',NOT$groupV,'$')` is generated from 
                          a normal distribution of mean 0 and variance `r  paste0('$Var(',NOT$groupV,')$')`, 
                          and the residual term `r  paste0('$',NOT$error,'$')` from a normal distribution 
                          of mean 0 and variance `r  paste0('$Var(',NOT$error,')$')`.",
  ...
  "samplingDesign_2"    = "First we define the duration of the sampling period that is the same for all individuals.
                          `r paste0($$t_{sampling}=t_{total}\\times(1-',general_VAR$btwIndVarTimSamp,')$$')`
                          where $t_{total}$ is the total simulation duration and `r  paste0('$',general_VAR$btwIndVarTimSamp,'$')` 
                          is the among-individual variance in timing of sampling (between 0 and 0.95).  
                          `r  paste0('$',general_VAR$btwIndVarTimSamp,'$')` is used to control the spreading intensity 
                          in sampling time between individuals (figure 1). 
                          When `r  paste0('$',general_VAR$btwIndVarTimSamp,'$')` is small (e.g. 0.1) $t_{sampling}$ is large 
                          (90% of total simulation duration) and all individuals are sampled almost fullmodelTxt
                          within the same period. Note that when `r  paste0('$',general_VAR$btwIndVarTimSamp,'$')` is 0 all individuals 
                          are sampled with the entire period of the simulation. 
                          Contrarily, when `r  paste0('$',general_VAR$btwIndVarTimSamp,'$')` is large (e.g. 0.9) $t_{sampling}$ is small 
                          (10% of total simulation duration) and each individual sampling period is spread out
                          within the total simulation duration following a uniform distribution.",
  "samplingDesign_3"    = "The number of sampled records for each individual (`r paste0('$NR_',NOT$ind,'$')`) is generated following 
                          a poisson distribution.
                          `r paste0('$$NR_',NOT$ind,'\\sim poisson(NR)$$')`
                          where $NR$ is the mean number of records.",
  ...
  "bivariateStepbyStep" = "SQuID can model a bivariate phenotypic equation with random intercepts 
                                 ...
                                 a focal environmental effect is set to affect both traits (e.g., `r  paste0('$',EQ$mean1.1,'\\neq 0$')` and `r  paste0('$',EQ$mean1.2,'\\neq 0$')`). 
                                 A large number of covariance terms, listed here, can thus be simulated.",
  
  # Step by Step bivariate section
  "summaryStepbyStep" = "In the following table, we summarized the components that can be extracted from a mixed-effects model, 
                               using a data set generated by SQuID in a situation where the phenotypic trait y 
                               is a function of two different environmental variables `r  paste0('$',EQ2$env1,'$')` and `r  paste0('$',EQ2$env2,'$'`) and their interaction.
  														 The phenotype trait `r  paste0('$',NOT$trait.1,'$')` is expressed for each individual `r  paste0('$',NOT$ind,'$')` and at each instance of time `r  paste0('$',NOT$time,'$')`, in each high-level group`r NOT$group`.",
  ...
  "inputEnvironment_1"    = "You have now to decide what types of environmental effects you will generate for the simulated traits. 
                            In this section you can select a maximum of two environment effects `r  paste0('$',EQ2$env1,'$')` and `r  paste0('$',EQ2$env2,'$')`). 
                            ...
                            structure by respectively checking or unchecking its associated checkbox.",
  ...
  "inputEnvironment_5"    = "Furthermore, below each environmental option a graph of the general environmental 
                            ...
                            then add further complexity by combining multiple environmental effect types.
  													...
  													environmental effect `r  paste0('$',EQ2$env1,'$')`, `r  paste0('$',EQ2$env2,'$')` and `r  paste0('$',EQ2$env12,'$')`) is 
  													expressed in unit variance (i.e., `r  paste0('$Var(',NOT$env,'=1)$')` and mean-centered (i.e., `r  paste0('$E(',NOT$env,')=0$')`.
  													However, the previsualization graph below displays the generated environmental data before standardization.",
  "inputEnvironment_6"    = "Finally, you can add a third environmental effect corresponding to the 
                            interaction between `r  paste0('$',EQ2$env1,'$')` and `r  paste0('$',EQ2$env2,'$')`. 
                            Note that an environmental interaction is allowed only when `r  paste0('$',EQ2$env1,'$')` 
                            and `r  paste0('$',EQ2$env2,'$')` are selected.",
  ...
  "inputSamplingDesign_3"= "On the right hand side of the section, there are little check boxes that 
                            ...
                            at the same time.",
  "inputSamplingDesign_4"=  "Finally, SQuID offers a previsualization of the sampling design. 
                            The figure shows when the trait `r  paste0('$',NOT$trait.1,'$')` of the first 5 individuals will be sampled. 
  ...
  "output_figure_color" = "The figures below show the data related to one trait (`r  paste0('$',NOT$trait.1,'$')`) and one 
...
module1_txt <- list(
  "title"         = "Partitioning variances: Differences between `r  paste0('V_',NOT$total,'$')`, `r  paste0('$V_',NOT$devI,'$')` and 
...
Mod1Step1_txt <- list(
  ...
  "intro"      = "<b>Introduction:</b> This is the simplest possible situation. 
                       A trait as measured for a group of individuals from a population 
                       is characterised by a mean (`r  paste0('$',NOT$mu,'$')`) and a variance (`r  paste0('$V_',NOT$total,'$')`).",
  ...
  "para1"      = "In every case that we measure something, we know that we are making an error in the measurement. 
                       This error is assumed to be non-directional and hopefully represents only a small portion of 
                       the total variance `r  paste0('V_',NOT$total,'$')`. Below play with the error term. 
                       ...
                       can be associated with much higher measurement error.",
  
  "explanation1"  = "An explanation of notation: There are several kinds of unaccounted variance 
                          ...
                          To distinguish these with notation, we will use `r  paste0('V_',NOT$mError,'$')` for measurement error 
                          and `r  paste0('V_',NOT$residualUpper,'$')` for the more general residual variance. In this particular step, 
                          `r  paste0('V_',NOT$residualUpper,'=V_',NOT$mError,'$')`. ",
  
  "note1"      = "Note that in this module, the total phenotypic variance `r  paste0('$(V_',NOT$total,'')$')` is restrained to 1. 
                       This will allow a better understanding of the proportions of the different model variance components.",
  
  "point"      = "<b>Point:</b> The variance of measures `r  paste0('$(V^\\prime_',NOT$total,')$')` is often higher than the variance 
                       of true individual variance `r  paste0('$(V^\\prime_',NOT$devI,')$')` if there is measurement error 
                       (and there will always be some). The estimate of total variance includes 
                       both individual differences `r  paste0('$(V_',NOT$devI,')$')` (not measured directly) and measurement error `r  paste0('$(V_',NOT$mError,')$')`. 
                       Note also that the estimated `r  paste0('$V^\\prime_',NOT$total,'$')` (total variance of sampled values) 
                       ...
                       The difference thus arises from sampling variance.",
  ...
  "statmodel2"  = "where `r  paste0('$',NOT$trait.1,'_{',NOT$time,NOT$ind,'}$')` is the measured phenotypic 
                          value of the $h^{th}$ measurement 
                         (in this case `r  paste0('$',NOT$time,'$')` = 1) on the $i^{th}$ individual, `r  paste0('$',NOT$devI,'_',NOT$ind,'$')` is the true deviation 
                         of the individual's trait from the population mean 
                         (which is assumed to be 0 for now), and `r  paste0('$',NOT$error,'_{',NOT$time,NOT$ind,'}$')`  is the residual 
                         deviation of that measurement from the true value, 
                         caused in this scenario by measurement error.",
  
  "statmodel3"  = "The second type of equation is a partitioning of variance 
                         ...
                         that in this scenario the variance in `r  paste0('$',NOT$error,'_{',NOT$time,NOT$ind,'}$')`, often called residual variance, 
                         is measurement variance `r  paste0('$(V_',NOT$mError,')$')`."
)

# Step 2 --------------
Mod1Step2_txt <- list(    
  "title"     = "Step 2: Repeatability and measurement error",
  "subgoal"   = "<b>Sub-goal:</b> Learning how to estimate measurement error variance `r  paste0('$(V_',NOT$mError,')$')`
  ...
  "exercise"    = "<b>Exercise:</b> To simulate this new situation, 
                  we advise using the same parameters as in the previous step, 
                  in terms of the number of individuals and measurement error 
                  (`r  paste0('$(V_',NOT$mError,')$')`; expressed as a proportion of the total phenotypic variance, `r  paste0('$(V_',NOT$total,')$')`) 
                  so that you can compare your output with the previous step:",
  ...
  "para3"   =  "The three density plots show the distribution of estimated total phenotypic variance `r  paste0('$(V^\\prime_',NOT$total,')$')`, 
                the estimated variance among individuals `r  paste0('$(V^\\prime_',NOT$devI,')$')`, and 
                the estimated variance within individuals `r  paste0('$(V^\\prime_',NOT$mError,')$')`. 
                It demonstrates graphically that `r  paste0('$V^\\prime_',NOT$total,'=V^\\prime_',NOT$devI,'+V'_',NOT$mError,'$')`.",
  ...
  "para6"   =  paste0("The scatter plot shows how correlated the first and second measurements are. 
                In the absence of measurement error (i.e., `r  paste0('$(V_^\\prime',NOT$mError,'=0$)')`, 
                all points align on a single line ($y=x$). Play around with the settings of `r  paste0('$(V_',NOT$mError,'$')`; 
...
Mod1Step3_txt <- list(  
  ...
  "exercise"  =  "<b>Exercise:</b> As in the two previous steps we will generate a new group 
                      ...
                      their true value `r paste0('$(V_',NOT$devI,')$')` and also due to observer measurement error `r paste0('$(V_',NOT$mError,')$')`. 
                      ...
                      This means that in addition to `r  paste0('$(V_',NOT$devI,'$')` and `r  paste0('$(V_',NOT$mError,'$')` there is variation caused by the population 
                      ...
                      Earlier we noted that `r  paste0('$(V_',NOT$residualUpper,'$')` is called &ldquo;residual variance&rdquo;. Now, unknown, 
                      environmentally-caused variance will be combined with `r  paste0('$(V_',NOT$mError,'$')` to make `r  paste0('$(V_',NOT$residualUpper,'$')`, 
                      ...
                      We will use the term `r  paste0('$(V_',NOT$envEffect,'$')` to refer to all phenotypic variance caused by the environment. 
                      Known environmental variance will be indicated by `r  paste0('$V_{',NOT$mean,' ',NOT$env,'}$.')`",
  
  "exercise2"  = "In this simulation, we assume there is only one environmental effect. 
                        We will specify it to generate phenotypes, and use the term `r  paste0('$V_{',NOT$mean,' ',NOT$env,'}$')`. 
                        You can thus try several combinations of `r  paste0('$(V_',NOT$mError,'$')`, `r  paste0('$(V_',NOT$devI,'$')`, and `r  paste0('$V_{',NOT$mean,' ',NOT$env,'}$')` 
                        ...
                        Just remember that for now `r paste0('$V_{',NOT$mean,' ',NOT$env,'}$')` is one environmental effect but 
                        it will be measured as part of the residual.",
  
  "para1"    = "Once again you can calculate repeatability as",
  
  "point"    = "<b>Point:</b> We can now estimate a particular variance component 
                of `r  paste0('$V_{',NOT$total,'}$')` that represents among-individual differences that are consistent through time. 
                ...
                `r  paste0('$V_{',NOT$envEffect,'}$')` in general reflects plasticity to unknown environments, 
                with `r  paste0('$V_{',NOT$mean,' ',NOT$env,'}$')` indicating variance due to a specified environmental variable `r  paste0('$',NOT$env,')$')`, 
  ...
  "statmodel2"  = "except that `r  paste0('$',NOT$error,' _{',NOT$time,NOT$ind,' }$')` contains both measurement 
                           error and deviations described by `r  paste0('$',NOT$mean,' ',NOT$env,' _{',NOT$time,NOT$ind,' }$')`",
  "RCode"       = "# install.packages(&quot;lme4&quot;)<br>
                  LMM <- lme4::lmer(Phenotype ~ 0 + (1|Individual), data = sampled_data)"
)
...
Mod1Step4_txt <- list( 
  ...
  "intro"     = "<b>Introduction:</b> In step 3, we introduced `r  paste0('$V_{',NOT$mean,' ',NOT$env,' }$')`
  ...
  "exercise"    = "<b>Exercise:</b> As before, we will generate a new group of individuals, 
                  with phenotypic variance caused by measurement error `r paste0('$(V_',NOT$mError,')$')`, individual differences `r paste0('$(V_',NOT$devI,')$')`, 
                  and specific and measured (i.e. known) environmental effects `r paste0('$(V_{',NOT$mean,' ',NOT$env,'})$')` on the phenotype of an individual. 
                  As before, you can set `r  paste0('$(V_',NOT$mError,'$)')`, `r  paste0('$(V_',NOT$devI,')$')` and `r  paste0('$V_{',NOT$mean,' ',NOT$env,' }$')`, but we suggest you use the same values you did in Step 3.
                  Remember that these variances must add up to 1.",
  
  "para1"    = "At this point, we want to expand on the idea of statistical models. 
                ...
                as well that `r  paste0('$',NOT$devI,'_',NOT$ind,'$')` is defined as the deviation of each individual from a population value. 
  ...
  "para2"    =  "where `r  paste0('$',NOT$trait.1,' _{',NOT$time,NOT$ind,' }$')` is the phenotype measured at the 
                     `r  paste0('$',NOT$time,'^{th}$')` time for the `r  paste0('$',NOT$ind,'^{th}$')` individual, 
                     `r  paste0('$',EQ3$mean0,'$')` is the population mean phenotype, 
                     `r  paste0('$',NOT$devI,'_',NOT$ind,'$')` is the individual mean deviation from the population mean for the `r  paste0('$',NOT$ind,'^{th}$')` individual, 
                     `r  paste0('$',NOT$mean,'$')` is the population mean effect of measured environment `r  paste0('$',NOT$env,' _{',NOT$time,NOT$ind,' }$')` on the measure of phenotype, 
                     and `r  paste0('$',NOT$error,' _{',NOT$time,NOT$ind,' }$')` is the error made in that measurement.",
  
  "note1"    =  "Note that in SQuID each environmental effect `r  paste0('$',NOT$env,')$')` is expressed in unit variance 
                      (i.e., `r  paste0('$Var(',NOT$env,')=1$')` and mean-centered (i.e., `r  paste0('$E(',NOT$env,')=0$)')`.
                      Then `r  paste0('$V_{',NOT$mean,' ',NOT$env,'}=Var(',NOT$mean,' ',NOT$env,')=',NOT$mean,'^2Var(',NOT$env,')=',NOT$mean,'^2$')`",
  
  "para3"    = "A mixed statistical model estimates these parameters:",
  "para4"    = "Visually, you can see what `r  paste0('$',NOT$mean,'$')` 
  ...
  "para6"    =  "We encourage you to go back and play around with the magnitude of `r  paste0('$(V_',NOT$mError,')$')` 
                to see how it affects estimates of  `r  paste0('$',NOT$mean,'$')`
                and `r  paste0('$(V_',NOT$devI,')$')`. You can also play around with the slope and the ratio of 
                `r  paste0('$V_{',NOT$mean,' ',NOT$env,' }$')` and `r  paste0('$(V_',NOT$devI,')$')` to better understand the effects.",
...
Mod3Step1_txt <- list(
  ...
  "intro"      =  "<b>Introduction:</b> In the previous module (<i>,`r Module_titles$mod1`,</i>), 
                        we partitioned phenotypic variance into several components 
                        (the variance among individuals, `r  paste0('$(V_',NOT$devI,')$')`, the variance caused by measurement error, `r  paste0('$(V_',NOT$mError,')$')`, 
                        and variance caused by the environment, `r  paste0('$(V_',NOT$envEffect,')$')`). In the final step of that module, 
  ...
  "exercise"   = "<b>Exercise:</b> As in previous simulations, we will generate a new group of individuals, 
                        with phenotypic variation generated by measurement error `r paste0('$(V_',NOT$mError,')$')`, individual differences `r paste0('$(V_',NOT$devI,')$')`, 
                        and the impact of the a specified environmental variable `r  paste0('$',NOT$env,'$')` which produces variance due 
                        to the environment `r paste0('$(V_',NOT$envEffect,')$')`. We have shifted to using the notation `r  paste0('$(V_',NOT$envEffect,')$')` here instead of `r  paste0('$V_{',NOT$mean,' ',NOT$env,' }$')`
                        which we used in Step 3 and 4 of the basic module &ldquo;<i> `r Module_titles$mod1`</i>&rdquo;. 
                        We do this because we will soon explore what happens when only some of the environmental 
                        variance is known, and we will use `r  paste0('$V_{',NOT$mean,' ',NOT$env,' }$')` for that known variance.",
  
  "para1"      = "As before, you can set `r  paste0('$(V_',NOT$mError,')$')`, `r  paste0('$(V_',NOT$devI,')$')` and `r  paste0('$V_{',NOT$envEffect,' }$')`, 
                        and for this module, `r  paste0('$V_{',NOT$envEffect,' }$')` must be greater than 0.",
  
  "note1"      = "Note that from now on, the total variance (`r  paste0('$(V_',NOT$total,')$')`) is not restrained to 1 anymore 
  ...
  "para2"      =  "The environment for this simulation is, for convenience, set as being linear over time, 
                  ...
                  The environment is also expressed in unit variance (i.e., `r  paste0('$Var(',NOT$env,')=1$')`
                  and mean-centered (i.e., `r  paste0('$E(',NOT$env,')=0$)`.",
  ...
  "para8"      = "The above should show that if the unmeasured environment changes over 
                        time AND there is among-individual variance in sampling, then some of the unknown 
                        `r  paste0('$(V_',NOT$envEffect,')$')` is placed into residual variance (making residual variance larger 
                        than just measurement variance `r  paste0('$(V_',NOT$mError,')$')`), and some ends up in the estimated 
                        `r  paste0('$(V_',NOT$devI,')$')`, also making it bigger than it should be.",
  "conclusion" = "<b>Conclusion</b>",
  "para9"      =  "This exercise demonstrates that if there is among-individual variance in timing of sampling, 
                        then estimates of `r  paste0('$(V_',NOT$devI,')$')` will be incorrect since inevitably there are systematic differences 
  ...
  "para10"     =  "Because among-individual variance in sampling and systematic changes in environment 
                  are extremely likely in real systems, how can we get accurate estimates of `r  paste0('$(V_',NOT$devI,')$')`?",
...
Mod3Step2_txt <- list(    
  ...
  "exercise"   = "<b>Exercise:</b> As in Step 1, we will generate a new group of individuals, 
                        with phenotypic variance caused by measurement error `r paste0('$(V_',NOT$mError,')$')`, individual differences `r paste0('$(V_',NOT$devI,')$')`, 
                        and the impact of the environment `r paste0('$(V_',NOT$envEffect,')$')`.",
  ...
  "para6"      =  "The results of any given simulation may vary, but the overall picture that emerges 
                         if you do several simulations should be that your estimates are better when `r  paste0('$(V_',NOT$envEffect,'$')` is small, 
  ...
  "para7"      =  "Did you simulate a population where the environment is not shared among individuals? 
                         ...
                         your estimate of `r  paste0('$(V_',NOT$devI,'$')` is too high. To understand, let's return to the definitions 
                         of the variance components: We defined `r  paste0('$(V_',NOT$devI,'$')` as the variance among individuals that permanently 
                         ...
                         &ldquo;<i> `r Module_titles$mod4`</i>&rdquo; module).",
...
Mod3Step3_txt <- list(  
  ...
  "intro"      = "<b>Introduction:</b> Step 1 of this module illustrated that environmental 
                        effects on phenotypes can produce biases in estimates of among-individual variance `r paste0('$(V_',NOT$devI,')$')`.  
                        ...
                        &ldquo;<i>`r Module_titles$mod1`</i>&rdquo; module. Here we demonstrate that this can, 
                        under some circumstances, solve the bias in sampling problem.",
  "exercise1"  = "<b>Exercise 1:</b> This exercise follows the same structure as all of our other simulations so far. 
                        We will generate a group of individuals, with phenotypic variance caused by measurement error `r paste0('$(V_',NOT$mError,')$')`, 
                        individual differences `r paste0('$(V_',NOT$devI,')$')`, and the impact of the environment `r paste0('$(V_',NOT$envEffect,')$')`. 
  ...
  "results"    = "<b>Results:</b> In the module &ldquo;<i>`r Module_titles$mod1`</i>&rdquo;, 
  ...
  "para6"      = "This should show you that when there is among-individual variance in sampling 
                        and you can account for all the environmental variance with an x variable, 
                        any bias in `r  paste0('$(V_',NOT$devI,'$')` caused by the biased sampling disappears.",
  "reminder"   = "A brief reminder about notation: When unknown environments affect phenotypic variance, 
                        we have referred to that variance as `r  paste0('$(V_',NOT$envEffect,'$')`.  In the model where the environment is known `r  paste0('$',NOT$env,')$')`, 
                        there now is a specific component of variance due to that known environmental factor, 
                        `r  paste0('$V_{',NOT$mean,' ',NOT$env,' }$')`. In the case above, `r  paste0('$(V_',NOT$envEffect,'=V_{',NOT$mean,' ',NOT$env,' })$')`, 
                        but in the real world with many environmental variables, 
                        `r  paste0('$V_{',NOT$mean,' ',NOT$env,' }$')` will be only a fraction of `r  paste0('$(V_',NOT$envEffect,'$)')`.",
  ...
  "conclusion2" = "The second lesson is that bias in sampling may occur without 
                         ...
                         The `r  paste0('$(V_',NOT$devI,'$)')` that is found from real data must therefore be interpreted cautiously.",
  "finalcaveat" = "<b>A final caveat:</b> An interesting consequence of having variance 
                         ...
                         If so, the method we have demonstrated here will not give accurate estimates of `r  paste0('$(V_',NOT$devI,'$)')`. 
...
Mod6Step1_txt <- list(
  ...
  "intro"      =  "<b>Introduction:</b> Modules &ldquo;<i>,`r Module_titles$mod1`</i>&rdquo; and &ldquo;<i> `r Module_titles$mod3` </i>&rdquo; 
                        have explored how among-individual `r paste0('$(V_',NOT$devI,')$')` variance in mean values can be modelled. 
                        In Step 3 of module &ldquo;<i>`r Module_titles$mod1`</i>&rdquo;, individuals also varied, to some extent, 
                        ...
                        to environmental fluctuations `r paste0('$(V_{',NOT$mean,'})$')`. Throughout all these lessons, 
  ...
  "exercise"  =  "<b>Exercise:</b> Here we depart from the random-intercept regression model presented in Step 4 of module 
                       &ldquo;<i>`r Module_titles$mod1`</i>&rdquo; by introducing a parameter that allows defining individual 
  ...
  "para2"    =  "in which `r  paste0('$',NOT$devS,'_',NOT$ind,'$')` is the effect of measured environment 
                `r  paste0('$',NOT$env,' _{',NOT$time, NOT$ind,' }$')`
...
Mod6Step2_txt <- list(    
  ...
  "para3"      =  "This means that the intercept value of the <i>i <sup>th</sup></i> individual `r  paste0('$',NOT$devI,'_',NOT$ind,'$')` 
                        and the slope of that individual `r  paste0('$',NOT$devS,'_',NOT$ind,'$')` are distributed as multivariate 
                        normal with means for each attribute of 0 and a variance-covariance structure 
                        of `r  paste0('$\\Omega_{',NOT$devI,NOT$devS,' }$')`. We then specify the variance-covariance structure in a 2 by 2 matrix. 
                        You have seen the variance terms (`r  paste0('$(V_',NOT$devI,')$')`  and `r  paste0('$(V_',NOT$devS,')$')` ) before, 
                        but the new term introduced here is `r  paste0('$Cov_{',NOT$devI, NOT$devS,' }$')`, which is the measure of 
                        how much `r  paste0('$',NOT$devI,'_',NOT$ind,'$')` and `r  paste0('$',NOT$devS,'_',NOT$ind,'$')` covary together.",
  ...
  "results"    = "<b>Results:</b> A mixed-effects statistical model estimates these parameters:",
  "note1"      = "Note that we have slightly changed what is presented in the table above. 
                        ...
                        factor, or `r  paste0('$V_{',NOT$devS,NOT$env,' }$')`, above we present what many statistical packages produce, 
                        which in the case of random slope, is the variance in slope `r paste0('$(V_',NOT$devS,')$')` 
  ...
  "note2"      = "Note: In SQuID `r  paste0('$E(',NOT$env,')=0$')` and hence the covariance does not contribute to total phenotypic variance.",
...
Mod6Step3_txt <- list(  
  ...
  "subgoal"    = "<b>Sub-goal:</b> to develop understanding of how the level of replication 
                        in terms of the number of individuals and samples per individual 
                        influence the estimation of the among-individual variance in intercepts `r paste0('$(V_',NOT$devI,')$')`, 
                        slopes `r paste0('$(V_{',NOT$devS,'})$)` and their correlation `r  paste0('$(Cor_{',NOT$devI,',',NOT$devS,'})$)`.",
  ...
  "para1"      = "For these simulations we are going to use a stochastic environment effect `r  paste0('$',NOT$env,'$')`. 
...
portal_txt <- list(
  ...
  "parag3_contents2"     =  "<b>SQuID</b> is based on a mathematical model that creates a group of individuals (i.e. study population) 
                                ...
                                phenotypic variance (`r  paste0('$(V_',NOT$total,'$')`) is assumed to be the sum of a series of components (see the full model). 
  ...
  "beginners"            = "The SQuID modules are designed for users who have some but not a 
                                ...
                                introduces multiple regression. The module &ldquo; `r Module_titles$mod6` &rdquo; 
