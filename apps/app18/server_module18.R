...
c(
    ...
    output$Mod4Step2_correlationplot <- renderPlot({
      ...
      if (!is.null(data)) {
        ...
        dt <- copy(data)
          ...
          ylab("Phenotype of trait z") + theme(legend.position="none") + ggtitle("With among-individual variances")
    	...
    	if (!is.null(data)) {
    		...
    		dt[ , ':='(Phenotype_1 = Phenotype_1 - mean(Phenotype_1), 
    		  ...
    		  ggplot(dt, aes(x = Phenotype_1, Phenotype_2,  fill = as.factor(Individual), colour = as.factor(Individual))) +
    			geom_point() +
    			xlab("Phenotype of trait y") +
    			ylab("") + theme(legend.position="none") + ggtitle("No among-individual variance")
  ...
  output$Mod4Step2_Among_Covariance_Matrix <- renderUI({
    ...
    output$Mod4Step2_Repeatability1 <- renderUI({
  ...
  output$Mod4Step2_Repeatability2 <- renderUI({
    
    rep2 <- round(input$Mod4Step2_Vi2 / (input$Mod4Step2_Vi2 + input$Mod4Step2_Ve2), 3)
    
    rep <- paste0("$$\\text{Repeatability of trait }",NOT$trait.2, "= ",rep2,"$$")
    
    return(withMathJax(rep))
  }),
    
  #by default hide
  hide("Mod4Step2_Among_Covariance_Matrix"),
  hide("Mod4Step2_Within_Covariance_Matrix"),
  hide("Mod4Step2_Phenotopic_correlation"),
  hide("Mod4Step2_Phenotopic_correlation2"),
  
  
  # Display hidden results
  observeEvent(input$show, {
    
    show("Mod4Step2_Among_Covariance_Matrix")
    show("Mod4Step2_Within_Covariance_Matrix")
    show("Mod4Step2_Phenotopic_correlation")
    show("Mod4Step2_Phenotopic_correlation2")
