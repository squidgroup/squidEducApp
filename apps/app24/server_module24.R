...
c(
    ...
    Mod6Step1_output <- reactive({
      ...
      isolate({ 
        ...
        LMR      <- lmer(Phenotype ~ 1 + X1 + (1|Individual) + (0+X1|Individual), data = data$sampled_data)
        FIXEF    <- fixef(LMR)
    ...
    Mod6Step1_table <- reactive({
      ...
      myTable <- data.frame(
        "Parameter"   = c("$\\text{Fixed effects}$",
                         ...
                         paste("Variance due to individual-specific responses to an environmental factor (random slopes; $V_{",NOT$devS,"}$) ="),
                         paste("Residual variance ($V_",NOT$residualUpper,"$) =")
                         ),
        "Truth"       = c("",
                        round(input$Mod6Step1_B[1],2),
                        round(input$Mod6Step1_B[2],2),
                        "",
                        input$Mod6Step1_Vi,
                        input$Mod6Step1_Vs,
                        input$Mod6Step1_Ve),
    ...
    output$Mod6Step1_plot <- renderPlot({ 
      ...
      if(!is.null(data)){
        ...
        print(
          ggplot(data = data$sampled_data, 
                          aes(y     = stats::predict(data$LMR), 
                                       x     = X1, 
